...
// Downloads releases from Fastly.
//
// "FastlyCache" is based on the earlier "ArtifactoryCache" and "BinTrayCache".
...
//
// With artifactory and fastly, the metadata is being found in the corresponding json file for each archive.
...
class FastlyCache {
    ...
    function __construct() {
        $this->path = Settings::dataPath('fastly');
        // Get a copy of boost
    ...
    static function discoverVersion($version) {
        ...
        $url = "https://archives.boost.io/$version/";
        $discovered_data=file_get_contents($url);
        $discovered_lines = explode("\n", $discovered_data);
        $saved_uri="";
        // find the largest version number
        foreach($discovered_lines as $line) {
            // <a href="boost_1_74_0-snapshot.7z">boost_1_74_0-snapshot.7z</a>  
            if (preg_match('/^<a\s*href="(.*)"\s*>/', $line, $matches)) {
                $uri=$matches[1];
    ...
    function fetchDetails($bintray_version, $bintray_path = null) {
        ...
        //    $filter_by_version = true;
            ...
            $filter_by_version = true;
            $fastly_repo = "beta";
            $fastly_file_path = "$directory_version/source";
        ...
        foreach ($extensions as $extension) {
            ...
            $url = "https://archives.boost.io/$fastly_repo/$fastly_file_path/$filename.json";

            $file_info = file_get_contents($url, false, $this->stream_context);
            if (!$file_info) {
                throw new RuntimeException("Error downloading file details from fastly.");
            }

            $file_info = json_decode($file_info);
            $file_info->name = $file_info->file;
            // $file_info->path = $fastly_file_path . "/" . $filename;
            $file_info->path = join("/", array_filter(array($fastly_file_path, $filename), 'strlen'));
            $file_info->repo = $fastly_repo;
            ...
            if (($fastly_repo == "develop") || ($fastly_repo == "master")) {
            ...
            if (isset($file_info->created) and $file_info->created) {
                ...
                if ($commit_date) {
                    ...
                    Log::warning("Unable to retrieve commit date. Check directories in var/data/fastly/_repo_\n");
                }
            }
            $files[] = $file_info;
            ...
            return new FastlyCache_FileDetails($this, $bintray_version, $bintray_path, $files);
    }
...
class FastlyCache_FileDetails {
    ...
    function getDownloadPage() {
        ...
        return FastlyCache::urlBaseDir($urls);
    }
    ...
    function getFileUrl($file) {
        ...
        return "https://archives.boost.io/{$file->repo}/{$file->path}";
    }
    ...
    function downloadFile($url, $dst_path) {
        ...
        try {
            ...
            system("curl --insecure -L -o $temp_path $url", $retval);
            if ($retval == 0) {
                rename($temp_path, $dst_path);
            }
            else {
                Log::info("The curl command was \n");
                Log::info("curl --insecure -L -o $temp_path $url \n");
